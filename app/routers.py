...
# 3.	Объявления пользователя id открыт GET
...
# 6.	Удалить свое Объявление закрыт DEL
...
# 5.	Рефрештокин закрыт POST
@api.route('/api/token/refresh', methods=['POST'])
@jwt_required(refresh=True)
def refresh():
    cur_user_id = get_jwt_identity()
    user = User.query.get(int(cur_user_id)).first()
    if not user:
        return jsonify({'message': 'User not found'}), 404
...
# 7.	Удалить свою закрыт DEL
@api.route('/api/user/<int:user_id>', methods=['DELETE'])
@jwt_required()
